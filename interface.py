...
class FinanceManagerUI:
    """
    Main GUI for PennyPal - Personal Finance Manager.


    Features:
    - Dashboard (auto-updating balance)
    - Transactions (add, delete, edit, filter)
    ...
    def __init__(self, root, db):
        ...
        self.categories = [
            "Groceries", "Gas", "Savings", "Entertainment",
            "Rent", "Miscellaneous"
        ]
    ...
    def setup_dashboard_tab(self):
        ...
        self.balance_label = tk.Label(
            self.dashboard_tab, text="Balance: $0.00",
        ...
        tk.Button(
            self.dashboard_tab, text="Refresh",
    ...
    def setup_transactions_tab(self):
        ...
        tk.Button(input_frame, text="Add Income",
                  command=lambda: self.add_transaction("income")).grid(row=3, column=0, pady=10)
        tk.Button(input_frame, text="Add Expense",
                  command=lambda: self.add_transaction("expense")).grid(row=3, column=1, pady=10)
        ...
        tk.Button(filter_frame, text="Apply Filter",
                  command=self.load_transactions).grid(row=0, column=3, padx=5)
        ...
        tk.Button(frame, text="Delete Selected Transaction",
                  command=self.delete_transaction).pack(pady=5)


        tk.Button(frame, text="Edit Selected Transaction",
    ...
    def save_transaction_edits(self):
        ...
        self.db.update_transaction(self.edit_id, amount, category, date)
    ...
    def add_transaction(self, t_type):
        ...
        self.db.add_transaction(amount, category, date, t_type)
    ...
    def load_transactions(self):
        ...
        rows = self.db.get_transactions_filtered(category, start, end)
        ...
        for tid, amount, cat, date, ttype in rows:
            ...
            self.transaction_list.insert(
                tk.END, f"{tid} | {cat} | {amount:+.2f} | {date}"
            )


        self.balance_label.config(text=f"Balance: ${balance:.2f}")
        self.update_dashboard()
        self.check_budgets(rows)
    ...
    def setup_budgets_tab(self):
        ...
        tk.Label(frame, text="Set Budgets by Category",
        ...
        tk.Button(frame, text="Save Budgets",
                  command=self.save_budgets).pack(pady=10)
    ...
    def setup_reports_tab(self):
        ...
        tk.Label(frame, text="Reports & Export",
                 font=("Helvetica", 16)).pack(pady=10)
        ...
        tk.Button(frame,
                  ...
                  command=self.show_bar_chart).pack(pady=10)
    ...
    def reset_data_warning(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm Reset",
            "⚠ WARNING: This will permanently delete ALL transactions.\n\nAre you sure?"
        )
        if confirm:
            self.reset_all_data()
    ...
    def reset_all_data(self):
        """Clear the database and UI."""
        self.db.reset_database()
        self.load_transactions()
        messagebox.showinfo("Database Reset", "All data has been deleted.")
